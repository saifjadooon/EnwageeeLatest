...
class SearchableSpinnerNew(private val context: Context) : LifecycleObserver {
    ...
    fun makeApiCallAndUpdateSpinner(query: String) {
        ...
        try {
            ApiUtils.getAPIService(context).SearchClientbyNamewithquery(
                token.toString(), query, false
            )
                    ...
                    override fun onResponse(
                        ...
                        if (response.body() != null) {
                            ...
                            for (i in 0 until response.body()!!.data.size) {
                                itemslist.add(response.body()!!.data.get(i).name)
                            }

                            global.searchedclientdata  =  response.body()
                            datalist= itemslist
                            isfromsearch = true
                        //    setSpinnerListItems(itemslist)
                            val adapter = SpinnerRecyclerAdapter(context,  datalist, object : OnItemSelectListener {
                                override fun setOnItemSelectListener(position: Int, selectedString: String) {
                                    selectedItemPosition = position
                                    selectedItem = selectedString
                                    if (dismissSpinnerOnItemClick) dismiss()
